<?php 
// api.php
// Versión 1.1: Mejorado el manejo de errores y estandarización de respuestas JSON.
...
switch ($action) {
  ...
  case 'get_config':
    echo json_encode(['headerTitle' => APP_HEADER_TITLE, 'logoPath' => APP_LOGO_PATH]);
    break;
  
  case 'get_public_config':
    echo json_encode(['headerTitle' => PUBLIC_HEADER_TITLE, 'logoPath' => PUBLIC_LOGO_PATH]);
    break;
  ...
  case 'upload':
    try {
        ...
        if (!$name || !$date || !$file || $file['error'] !== UPLOAD_ERR_OK) { 
            http_response_code(400); 
            send_json_response(false, 'Faltan datos o hubo un error al subir el archivo.'); 
        }
        $filename = time().'_'.preg_replace("/[^a-zA-Z0-9\._-]/", "_", basename($file['name']));
        if (!move_uploaded_file($file['tmp_name'], __DIR__.'/uploads/'.$filename)) { 
            http_response_code(500); 
            send_json_response(false, 'No se pudo guardar el archivo en el servidor.'); 
        }
        ...
        send_json_response(true, 'Documento guardado exitosamente.');
    } catch (PDOException $e) {
        http_response_code(500);
        send_json_response(false, 'Error en la base de datos al guardar el documento.', [], $e->getMessage());
    }
    break;
  ...
  case 'list':
    ...
    send_json_response(true, 'Lista de documentos obtenida.', ['data' => $docs]);
    break;

  case 'search':
    // Lógica sin cambios...
    $codes_to_find = array_filter(array_unique(array_map('trim', preg_split('/[\\r\\n,]+/', $_POST['codes'] ?? ''))));
    if (empty($codes_to_find)) { echo json_encode([]); exit; }
    $placeholders = implode(',', array_fill(0, count($codes_to_find), '?'));
    $stmt = $db->prepare("SELECT d.id, d.name, d.date, d.path, GROUP_CONCAT(c.code SEPARATOR '|') AS codes FROM documents d JOIN codes c ON d.id = c.document_id WHERE c.code IN ($placeholders) GROUP BY d.id, d.name, d.date, d.path ORDER BY d.date DESC");
    $stmt->execute($codes_to_find);
    $all_docs_raw = $stmt->fetchAll();
    $candidate_docs = [];
    foreach ($all_docs_raw as $doc) { $candidate_docs[$doc['id']] = ['id' => (int)$doc['id'], 'name' => $doc['name'], 'date' => $doc['date'], 'path' => $doc['path'], 'codes' => $doc['codes'] ? explode('|', $doc['codes']) : []]; }
    $remaining_codes = array_flip($codes_to_find);
    $selected_docs = [];
    while (!empty($remaining_codes) && !empty($candidate_docs)) {
        $best_doc_id = -1; $max_covered_count = -1;
        foreach ($candidate_docs as $doc_id => $doc) {
            ...
            if ($covered_count > $max_covered_count) { $max_covered_count = $covered_count; $best_doc_id = $doc_id; }
        }
        if ($best_doc_id === -1 || $max_covered_count === 0) break;
        $best_doc = $candidate_docs[$best_doc_id]; $selected_docs[$best_doc_id] = $best_doc;
        foreach ($best_doc['codes'] as $code) { unset($remaining_codes[$code]); }
  ...
  case 'download_pdfs':
    // Lógica sin cambios...
    $uploadsDir = __DIR__ . '/uploads'; if (!is_dir($uploadsDir)) { http_response_code(500); send_json_response(false, 'La carpeta de subidas no fue encontrada.'); }
    $zip = new ZipArchive(); $zipFileName = 'documentos_'.date('Ymd_His').'.zip'; $zipFilePath = sys_get_temp_dir() . '/' . $zipFileName;
    if ($zip->open($zipFilePath, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== TRUE) { http_response_code(500); send_json_response(false, 'No se pudo crear el archivo ZIP.'); }
    ...
    foreach ($files as $file) { if (!$file->isDir()) { $filePath = $file->getRealPath(); $relativePath = substr($filePath, strlen($uploadsDir) + 1); $zip->addFile($filePath, $relativePath); } }
    $zip->close(); header('Content-Type: application/zip'); header('Content-Disposition: attachment; filename="' . basename($zipFileName) . '"'); header('Content-Length: ' . filesize($zipFilePath));
    readfile($zipFilePath); unlink($zipFilePath);
    exit;
  ...
  case 'edit':
    try {
        ...
        $name = $_POST['name'] ?? '';
        $date = $_POST['date'] ?? '';
        if (!$id || !$name || !$date) { http_response_code(400); send_json_response(false, 'Faltan datos para la actualización.'); }
        $codes = array_filter(array_map('trim', preg_split('/\\r?\\n/', $_POST['codes'] ?? '')));
        if (!empty($_FILES['file']['tmp_name'])) {
          $old = $db->prepare('SELECT path FROM documents WHERE id=?'); $old->execute([$id]);
        ...
        send_json_response(true, 'Documento actualizado correctamente.');
    } catch (PDOException $e) {
        http_response_code(500);
        send_json_response(false, 'Error en la base de datos al actualizar.', [], $e->getMessage());
    }
    break;
  ...
  case 'delete':
    try {
        $id = (int)($_GET['id'] ?? 0);
        if (!$id) { http_response_code(400); send_json_response(false, 'El ID del documento no es válido.'); }
        $old = $db->prepare('SELECT path FROM documents WHERE id=?'); $old->execute([$id]);
        if ($pathToDelete = $old->fetchColumn()) { @unlink(__DIR__.'/uploads/'.$pathToDelete); }
        $db->prepare('DELETE FROM codes WHERE document_id=?')->execute([$id]);
        $db->prepare('DELETE FROM documents WHERE id=?')->execute([$id]);
        send_json_response(true, 'Documento eliminado exitosamente.');
    } catch (PDOException $e) {
        http_response_code(500);
        send_json_response(false, 'Error en la base de datos al eliminar.', [], $e->getMessage());
    }
    break;
  ...
  case 'highlight_pdf':
    ...
    if ($httpCode === 200 && strpos($responseHeaders['content-type'][0], 'application/pdf') !== false) {
